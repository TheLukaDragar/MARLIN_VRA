...
class VRABase(LightningDataModule, ABC):
    ...
    def __init__(self, data_root: str, split: str, task: str, data_ratio: float = 1.0, take_num: int = None):
        ...
        self.metadata = read_json(os.path.join(data_root, "vra_metadata.json"))
...
class CelebvHq(VRABase):
    ...
    def __getitem__(self, index: int):
        ...
        return video, torch.tensor(y, dtype=torch.float32)


# For linear probing
class CelebvHqFeatures(VRABase):
...
class VRADataModule(LightningDataModule):
